...
class MonsterSeed:
    ...
    # Canonical form/species (new terminology: `form`). Kept as `species` property for backward compatibility.
    form: str
    ...
    def forge(
        ...
        forms_raw = SEED_TYPE_DATA.get(primary_type, {}).get("forms") or FORMS_BY_TYPE.get(primary_type, [])
        ...
        def _mod_allowed(mod: dict, primary: str, secondary: Optional[str]) -> bool:
            ...
            if allowed is None:
                return True
            return (primary in allowed) or (secondary in allowed if secondary else False)
        ...
        # If no compatible mods are available, fall back to the unfiltered pools.
        if not major_weights:
            major_weights = {name: mod.get("rarity", 1.0) for name, mod in MAJOR_MODS.items()}
        if not utility_weights:
            utility_weights = {name: mod.get("rarity", 1.0) for name, mod in UTILITY_MODS.items()}
...
def get_base_meta(
    ...
    def species(self) -> str:
        """Backward-compatible alias for `form`. Returns the canonical form name."""
        return self.form
