...
# ---------- Config ----------
TRIALS = 20000
MONSTER_TYPE = "Insectoid"  # change to a type present in your data
...
def build_expected_weights_for_seed(seed) -> Dict[str, float]:
    ...
    # seed elements set for synergy checks (strings)
    seed_element_set = set()
    for bucket in seed.elements.values():
        for e in bucket:
            seed_element_set.add(str(e))
    ...
    for key, mod_def in MAJOR_MODS.items():
        ...
        for s in mod_def.get("synergy", []) or []:
            if str(s) in seed_element_set:
                synergy_mult *= float(
                    mod_def.get("synergy_factor", mon_forge.DEFAULT_SYNERGY_FACTOR)
                )
...
def main():
    ...
    counts = Counter()
    cumulative_expected = defaultdict(
        float
    )  # sum of expected probabilities per mod across trials
    none_count = 0
    ...
    for i in range(TRIALS):
        ...
        m = mon_forge.generate_monster(
            idnum=i,
            primary_type=MONSTER_TYPE,
            major_count=1,
            util_count=0,
            secondary_type=None,
        )
        majors = m.elements.get("major", [])
        if majors:
            counts[majors[0]] += 1
        else:
            none_count += 1
    ...
    for k in sorted(
        all_keys, key=lambda x: (-avg_expected.get(x, 0), -counts.get(x, 0))
    ):
