...
def test_forge_seed_monster_dual_type():
	...
	monster = forge_seed_monster(idnum=11, primary_type="Inferno", secondary_type="Mineral")
	assert monster is not None
...
def test_generate_monster_full_process():
	...
	"""
	monster = generate_monster(idnum=99, primary_type="Inferno", major_count=1, util_count=1)
	assert monster is not None
	assert monster.primary_type == "Inferno"
...
def test_prompt_engine():
	...
	monster = generate_monster(idnum=42, primary_type="Aerial", major_count=1, util_count=1)
...
def test_all_monster_types_generate(monster_type):
	...
	try:
		monster = generate_monster(idnum=1, primary_type=monster_type, major_count=1, util_count=1)
		assert monster is not None
