...
def main():
	parser = argparse.ArgumentParser(description="A command-line tool for generating fantasy monsters.",
	...
	parser_dex.add_argument("-maj", "--majors", type=int, default=1, choices=range(3), help="Number of major mutagens.", )
	parser_dex.add_argument("-ut", "--utils", type=int, default=1, choices=range(2), help="Number of utility mutagens.", )
	parser_dex.add_argument("-c", "--count", type=int, default=1, help="Number of monsters to generate.")
	parser_dex.add_argument("-o", "--output", type=str, default="C:\\Users\\Bean\\Desktop\\PokeDex\\mongen_dexentry.txt",
	...
	parser_unique.add_argument("-t2", "--secondary_type", type=str, default="random", choices=SEED_TYPES + ["random"] + [None],
	...
	parser_prompt.add_argument("-t1", "--primary_type", type=str, default="random", choices=SEED_TYPES + ["random"],
		help="Primary type " "for the " "monster.", )
	...
	parser_prompt.add_argument("-o", "--output", type=str, default="C:/Users/Bean/Desktop/PokeDex/art_prompts.txt",
	...
	def get_monster_types(primary_arg, secondary_arg):
		...
		else:  # primary_arg must be in SEED_TYPES
			p_type = primary_arg

		if p_type:
			if secondary_arg == "random" and random.random() <= 0.65:
	...
	if args.command == "dexentry":
		...
		for i in range(args.count):
			...
			try:
				monster_template = generate_monster(idnum=i + 1, primary_type=p_type, secondary_type=s_type, major_count=args.majors, util_count=args.utils)
		...
		p_type, s_type = get_monster_types(args.primary_type, args.secondary_type)
		try:
			wild_monster = generate_monster(idnum=1, primary_type=p_type, secondary_type=s_type, major_count=args.majors, util_count=args.utils, )
		...
		p_type, s_type = get_monster_types(args.primary_type, args.secondary_type)
		try:
			monster_seed = generate_monster(idnum=1, primary_type=p_type, secondary_type=s_type, major_count=args.majors, util_count=args.utils, )
