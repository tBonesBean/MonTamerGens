...
DEBUG = bool(os.getenv("DEX_DEBUG"))  # Optional debug toggle (set env var DEX_DEBUG=1 to enable)
...
def forge_seed_monster(idnum: int, primary_type: str, secondary_type: Any) -> MonsterSeed:
	"""Factory function that uses the MonsterSeed's own forge method."""
	seed = MonsterSeed.forge(idnum, primary_type, secondary_type)
	return seed


def weighted_sample_without_replacement(weight_dict: Dict[str, float], k: int) -> List[str]:
	"""
...
def apply_mutagens(seed: MonsterSeed, major_count: int = 0, util_count: int = 0, ) -> MonsterSeed:
	"""
	...
	for key, mod_def in data.MAJOR_MODS.items():
		...
		for s in mod_def.get("synergy", []) or []:
			if str(s) in seed_mutagen_set:
				synergy_mult *= float(mod_def.get("synergy_factor", DEFAULT_SYNERGY_FACTOR))
		final_w = base_w * min(synergy_mult, MAX_SYNERGY_MULT)
		if final_w > 0.0:
			available_majors[key] = final_w
			dbg(f"major candidate: {key}, base_w={base_w:.6f}, synergy_mult={synergy_mult:.3f}, final_w={final_w:.6f}")

	available_utilities: Dict[str, float] = {}
	for key, mod_def in data.UTILITY_MODS.items():
		required = mod_def.get("required_types", []) or []
		incompatible = mod_def.get("incompatible_types", []) or []
		...
		if final_w > 0.0:
			...
			dbg(f"util candidate: {key}, base_w={base_w:.6f}, synergy_mult={synergy_mult:.3f}, final_w={final_w:.6f}")
	...
	chosen_utilities = weighted_sample_without_replacement(available_utilities, util_count)
	...
	for key in chosen_majors:
		mod_def = data.MAJOR_MODS.get(key)
		if mod_def:
			apply_one(mod_def)
	for key in chosen_utilities:
		mod_def = data.UTILITY_MODS.get(key)
		if mod_def:
			apply_one(mod_def)
...
def generate_monster(idnum: int, primary_type: str, secondary_type: Optional[str] = None, major_count: int = 0, util_count: int = 0, ) -> MonsterSeed:
	"""Combines forging and mutagen application into one call."""
	generic = forge_seed_monster(idnum, primary_type, secondary_type)
	seed = apply_mutagens(generic, major_count, util_count)
	return forge_monster_name(seed)
