...
def generate_dex_batch(count: int, major_count: int, util_count: int, output_path: str) -> list[str]:
	"""
	...
	for i in range(count):
		...
		while seed is None:
			try:
				...
				seed = forge_seed_monster(idnum=dex_number, primary_type=primary_type, secondary_type=secondary_type, )
			except ValueError:
				continue

		# Layer 2: apply mutagens to flesh out stats/mutagens/meta
		full_seed = apply_mutagens(seed, major_count=major_count, util_count=util_count, )
...
if __name__ == "__main__":
	...
	dexstack = generate_dex_batch(count=75, major_count=1, util_count=1, output_path="D:/Projects/MonsterDex/dex_batch_001.txt", )
