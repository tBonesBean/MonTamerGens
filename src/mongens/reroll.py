...
from . import mon_forge
from .monsterseed import MonsterSeed, weighted_choice, calculate_base_stats, get_base_meta
from .data.data import PHYSICAL_TRAITS, HELD_ITEMS, MAJOR_MODS, UTILITY_MODS

"""
This module contains the logic for re-rolling attributes of existing monsters.
This is achieved by loading a cached monster, surgically modifying its attributes,
and then re-running the necessary parts of the generation pipeline to ensure
consistency. The new monster is saved to the cache with a new PIN.
"""
...
def apply_mutagens_to_stats(base_stats: dict[str, int], base_meta: dict[str, list], mutagens: list[str]) -> tuple[dict[str, int], dict[str, any]]:
    """
    ...
    Args:
        ...
        base_meta: A dictionary of the monster's meta attributes (tags, resistances, etc.)
                   before any mutagens are applied.
        mutagens: A list of strings representing the keys of the mutagens to apply.

    Returns:
        A tuple containing the modified stats dictionary and the modified meta dictionary.
    """
...
def reroll_monster_attributes(pin: str, reroll_options: dict) -> MonsterSeed | None:
    ...
    if reroll_options.get('majors'):
        ...
        base_stats = calculate_base_stats(new_monster.primary_type, new_monster.secondary_type)
        ...
        for key, mod_def in MAJOR_MODS.items():
            ...
            available_majors[key] = mon_forge.rarity_to_weight(mod_def.get("rarity", 1.0))
        ...
        new_stats, new_meta = apply_mutagens_to_stats(base_stats, base_meta, all_mutagens)
