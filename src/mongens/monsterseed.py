...
def weighted_choice(choices_with_weights: ChoiceList):
	...
	for item, w in items_weights:
		try:
			...
			raise ValueError(f"weighted_choice: weight for {item!r} is not numeric: {w!r}")
		if w < 0:
	...
	raise RuntimeError("weighted_choice: failed to select an item (floating point edge-case)")
...
class MonsterSeed:
	...
	@classmethod
	def forge(cls, idnum: int, primary_type: str, secondary_type: str, ) -> "MonsterSeed":
		"""
		...
		seed = cls(idnum=idnum, name=name, species=species, primary_type=primary_type, secondary_type=secondary_type, stats=stats, mutagens=mutagens,
			habitat=habitat, tempers=tempers, traits=traits, meta=meta, )
