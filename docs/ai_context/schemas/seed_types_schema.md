...
-   The `tags` should be appropriately formatted `PREFIX:VALUE` strings that maintain coherence when processed by downstream systems (`Weak:Kinetic`, `Resist:Astral`, `Ability:TimeShift`, `Boost:Habitat`, etc.).
-   The `notes` should be high-level thematic keywords, not detailed sentences. Intended to guide narrative generation, as well as AI image generation.
-   The `habitats` should align with the theme of the seed `type` and potential forms from `type_forms.yaml`.
