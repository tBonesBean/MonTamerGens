...
- **YAML is authoritative for content, not logic**: Data files define _what_ exists (items, traits), while Python code defines _how_ they interact.
...
#### + TODO list (as of latest version of this document)
    ...
    {} Top priority should be assigned to populating the .py data sets and YAML content that sources an updated version of MonsterSeed with new attributes, values for those attributes, and new mutagens.

    {} Close second in priority: revisit the logic flow that drives generation, specifically the processes that rely on 'seed_types.yaml' and 'type_forms.yaml'.

    {} Assess the overall state of a MonsterSeed after reconciling with what results from the two TODO items above, to determine next steps.

    {} Make sure data sets that are being reworked are COMPLETELY reworked. Verify consistancy of shape in new/tweaked structures that constitue 'data.py' and any migrations to YAML files.
